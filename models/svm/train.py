...
def main():
    ...
    parser.add_argument(
        ...
        help='Kernel cho SVM (mặc định: linear - nhanh nhất cho dữ liệu lớn)'
    )
    parser.add_argument(
        '--C',
        type=float,
        default=1.0,
        help='Regularization parameter C (mặc định: 1.0)'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='models/saved/svm',
        help='Thư mục lưu model (mặc định: models/saved/svm)'
    )
    parser.add_argument(
        '--sample-size',
        type=int,
    ...
    # Train
    print("\n=== Bắt đầu training ===")
    print("Lưu ý: SVM có thể mất nhiều thời gian hơn các mô hình khác, đặc biệt với kernel='rbf'")
    results = model.train(
        X, y,
    ...
    print("  python models/svm/predict.py")
