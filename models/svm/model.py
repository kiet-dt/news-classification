...
class NewsSVM:
    """Mô hình SVM cho phân loại tin tức"""
    
    def __init__(self, max_features=10000, random_state=42, kernel='linear', C=1.0):
        """
        Args:
            max_features: Số lượng features tối đa cho TF-IDF
            random_state: Random seed
            kernel: Kernel cho SVM ('linear', 'rbf', 'poly', 'sigmoid')
                   Với dữ liệu lớn, nên dùng 'linear' (nhanh hơn)
            C: Regularization parameter (mặc định: 1.0)
        """
        ...
        # Với kernel='linear', dùng LinearSVC (nhanh hơn SVC)
        if kernel == 'linear':
            self.model = LinearSVC(
                C=C,
                random_state=random_state,
                max_iter=1000,  # Tăng max_iter nếu cần
                dual=False  # dual=False tốt hơn khi n_samples > n_features
            )
        else:
            self.model = SVC(
                kernel=kernel,
                C=C,
                random_state=random_state,
                probability=True,  # Cần True để có predict_proba
                max_iter=1000
            )
    ...
    def train(self, X, y, test_size=0.2, validation_size=0.1):
        ...
        # Train model
        print(f"Đang train model (kernel={self.kernel}, C={self.C})...")
    ...
    def predict(self, text, return_proba=False):
        ...
        # Predict
        if return_proba:
            # LinearSVC không có predict_proba, cần dùng decision_function
            if isinstance(self.model, LinearSVC):
                # Chuyển decision_function thành probability (softmax)
    ...
    def save(self, model_dir='models/saved/svm'):
        """Lưu model"""
    ...
    def load(self, model_dir='models/saved/svm'):
        """Load model"""
        model_path = os.path.join(model_dir, 'model.pkl')
        vectorizer_path = os.path.join(model_dir, 'vectorizer.pkl')
        label_encoder_path = os.path.join(model_dir, 'labels.pkl')
        ...
        with open(label_encoder_path, 'rb') as f:
            ...
            self.kernel = labels.get('kernel', 'linear')
            self.C = labels.get('C', 1.0)
