...
class NewsNaiveBayes:
    """Mô hình Naive Bayes cho phân loại tin tức"""
    
    def __init__(self, max_features=10000, random_state=42, alpha=1.0):
        """
        Args:
            max_features: Số lượng features tối đa cho TF-IDF
            random_state: Random seed
            alpha: Smoothing parameter cho Naive Bayes (mặc định: 1.0 - Laplace smoothing)
        """
    ...
    def save(self, model_dir='models/saved/naive_bayes'):
        """Lưu model"""
    ...
    def load(self, model_dir='models/saved/naive_bayes'):
        """Load model"""
        model_path = os.path.join(model_dir, 'model.pkl')
        vectorizer_path = os.path.join(model_dir, 'vectorizer.pkl')
        label_encoder_path = os.path.join(model_dir, 'labels.pkl')
